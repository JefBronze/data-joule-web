export const fr = {
  ...
  home: {
    ...
    flow_control_label: 'Agent de contrôle',
  ...
  jlc: {
    ...
    total_minted: 'Vérifié · émission en attente',
    ...
    chain_vtn_action: 'Émet événement + ID',
    chain_ven_action: 'Applique les paliers',
    chain_plug_action: 'Mesure physique',
    ...
    why_p2: 'Les Crédits Joule en sont les rails de règlement on-chain : un véritable événement OpenADR, une mesure physique de puissance, une vérification par oracle décentralisé de ce flux, et un plafond Proof-of-Reserve pour que l\'offre ne dépasse jamais les kWh attestés — chaque émission portant une piste d\'audit immuable (ID d\'événement, ID de requête, hash de transaction).',
    why_p3: 'Une confiance de niveau règlement exige que la mesure provienne d\'une source en laquelle la contrepartie a confiance — un compteur de qualité réseau et la répartition d\'un opérateur, et non l\'auto-déclaration d\'une seule partie. Les rails sont construits ; les ancrer à une métrologie de confiance et à un acheteur réel transforme un kWh vérifié en kWh réglé.',
    ...
    home_oracle_desc: 'Chaque événement d\'effacement terminé déclenche une requête de vérification on-chain. Plusieurs nœuds oracles indépendants récupèrent la mesure physique de puissance et atteignent un consensus, et un plafond Proof-of-Reserve maintient l\'offre dans les limites des kWh vérifiés.',
    ...
    mint_status: 'Aucun token émis pour l\'instant — c\'est voulu. L\'émission est conditionnée par une attestation Proof-of-Reserve indépendante (Chainlink CRE) ; tant qu\'elle n\'est pas active et à jour, le contrat refuse d\'émettre, donc l\'offre ne peut jamais dépasser les kWh vérifiés.',
    chain_por_note: 'La v2 ajoute un attestateur Proof-of-Reserve indépendant (Chainlink CRE) qui plafonne l\'émission aux kWh vérifiés — l\'offre ne peut jamais dépasser ce qui a été mesuré.',
    stat_pending_sub: 'émission on-chain en attente',
    ...
    economics_fleet: ['1 Pi (ce nœud)', '1 000 nœuds', '1M nœuds'],
    why_labels: ['Le Problème', 'Ce que nous avons bâti', 'La voie vers la valeur'],
  },
  method: {
